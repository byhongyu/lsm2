"""Creating low-level caption for SensorLM data."""

from typing import Dict, List
import numpy as np
from google3.medical.waveforms.modelling.lsm.datasets.sensorlm import sensor_constants as constants
...
def denormalize_sensor_values(
  ...
  Args:
      x: A np.ndarray of shape [N, len(features)], where N is the time length,
...
def generate_caption(
  ...
  Args:
      x_raw: A np.ndarray of shape [N, len(features)], where N is the time
