...
--config=experimental/largesensormodels/scenic/configs/lsm_v2_generative_eval/mae_lsm_small_generative_eval.py \
--platform=glp_1x1 \
--exp_name=lsm_v2_generative_evaluator \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/lsm_v2_generative_eval/mae_lsm_small_generative_eval.py:runlocal
"""
...
# http://xids/154207705
EXPERIMENT_NAMES = [
    'reassess_inheritmask_evalinheritfalse',
]
XM_ID = 153865496
WIDS = 2
INIT_CHECKPOINT_STEP = [20000]
...
################################################################################
INIT_CHECKPOINT_DIR = [f'{config_constants.CHECKPOINT_XM_DIR}/{XM_ID}/{WIDS}']
...
BATCH_SIZE = 4
# Random Imputation eval
ENABLE_DUMP_MODE = True
...
def get_config(runlocal=''):
  ...
  config.experiment_name = EXPERIMENT_NAMES[0]
  # config.dataset_name is unused, but needs to be set.
  config.dataset_name = 'lsm_v2_pretrain_sessions_-1_windowsize_1440_sensorfeatures_26_validonly_False_missingratio_0.5_timestamp_202503090320'
  ...
  # config.dataset_configs.dataset is unused, but needs to be set.
  config.dataset_configs.dataset = 'lsm_v2_pretrain_sessions_-1_windowsize_1440_sensorfeatures_26_validonly_False_missingratio_0.5_timestamp_202503090320'
  config.dataset_configs.valid_dataset = config_constants.VALID_DATASET_NAME

  if runlocal:
    config.output_dir = '/tmp'
  else:
    config.output_dir = config_constants.OUTPUT_RECONSTRUCTION_DIR
  config.dataset_configs.num_classes = None
  config.dataset_configs.train_split = 'train'  # train data split
  config.dataset_configs.train_num_samples = 0  # train sample
  # eval data split - note: this split is used for validation and test.
  config.dataset_configs.eval_split = 'valid[:16]' if runlocal else 'valid'
  ...
  # Training.
  config.trainer_name = 'lsm_v2_generative_evaluator'
  ...
  config.forecast = predefined_configs.LSM_PREDEFINED_CONFIGS['eval_fore_1day']
  config.imputation = predefined_configs.LSM_PREDEFINED_CONFIGS['eval_imp_1day']
  config.random_imputation = predefined_configs.LSM_PREDEFINED_CONFIGS[
      'eval_randimp_1day'
  ]
  # Logging.
...
def get_hyper(hyper):
  ...
  return hyper.product([
      hyper.sweep('config.experiment_name', EXPERIMENT_NAMES),
