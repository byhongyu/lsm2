r"""A config to run geneative evaluation a Extra Small ViT MAE on LSM dataset.
...
--config=experimental/largesensormodels/scenic/configs/lsm_generative_eval/mae_lsm_extrasmall_generative_eval.py \
--platform=vlp_4x4 \
--exp_name=lsm_mae_tier2_extrasmall_10_5_res_generative_task_eval \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/lsm_generative_eval/mae_lsm_extrasmall_generative_eval.py:runlocal
"""
...
RECONSTRUCTION_HORIZONS = [0.034, 0.067]  # 10, 20 mins
SENSOR_IMPUTATION_HORIZONS = [0.2, 0.4, 0.5]  # 5, 10, 15 sensors masked
...
def get_config(runlocal=''):
  ...
  config.experiment_name = f'electrodes-mae-{DATASET_NAME}_generative_eval'
  ...
  # Training.
  config.trainer_name = 'lsm_generative_evaluator'
  ...
  config.sensor_imputation = ml_collections.ConfigDict()
  config.sensor_imputation.horizons = SENSOR_IMPUTATION_HORIZONS
