...
--config=experimental/largesensormodels/scenic/configs/lsm_v2_pretraining/mae_lsm_v2_mt_small.py \
--platform=glp_8x16 \
--exp_name=25_4_6_inh_bi_1rand8_1bar4_largescale \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/lsm_v2_pretraining/mae_lsm_v2_mt_small.py:runlocal
"""
...
def get_config(runlocal=''):
  ...
  ######################## paste this into mae_lsm_small_generative_eval.py !!!!! ########################
  ...
  config.masker_config = Masker_Config(
      maskstrategy_list=[
          ...
          MaskStrategy_Config(
              strategy='bar', mask_probability=.4, weight=1, mask_dim='time',
              inherited_depend=False,
          ),
      ],
      on_cpu=True,
      inherited=True,
      strictmaskperc= 57/144, 
      mixstrategy="between_instance"
  )
