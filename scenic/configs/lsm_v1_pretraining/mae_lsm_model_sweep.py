...
--config=experimental/largesensormodels/scenic/configs/mae_lsm_model_sweep.py \
--platform=vlp_4x4 \
--exp_name=lsm_mae_tier2_sweep_10_5_res \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/mae_lsm_model_sweep.py:runlocal
"""
...
USE_TRAIN_AUGMENTATIONS = [False, True]
...
def map_variant_to_config(variant: str) -> dict[str, Any]:
  """Maps a variant to a config."""
  ...
  config['config.model_name'] = 'lsm_vit_mae'
  # encoder
  config['config.model.hidden_size'] = model_constants.HIDDEN_SIZES[version]
  config['config.model.patches.size'] = tuple([int(patch_h), int(patch_w)])
  config['config.model.num_heads'] = model_constants.NUM_HEADS[version]
  config['config.model.mlp_dim'] = model_constants.MLP_DIMS[version]
  config['config.model.num_layers'] = model_constants.NUM_LAYERS[version]
  # decoder
  config['config.model.decoder_config.hidden_size'] = (
      model_constants.DECODER_HIDDEN_SIZES[version]
  )
  config['config.model.decoder_config.mlp_dim'] = (
      model_constants.DECODER_MLP_DIMS[version]
  )
  config['config.model.decoder_config.num_layers'] = (
      model_constants.DECODER_NUM_LAYERS[version]
  )
  config['config.model.decoder_config.num_heads'] = (
      model_constants.DECODER_NUM_HEADS[version]
  )

  return config


def get_all_config_key() -> list[str]:
  """Returns all the config to be changed for one variant."""
  config = map_variant_to_config('S/10/5')
...
def get_hyper(hyper):
  ...
  experiment_name = hyper.sweep(
      'config.experiment_name',
      [
          f'electrodes-mae-{DATASET_NAME}-Deb/10/5',
          f'electrodes-mae-{DATASET_NAME}-TiShallow/10/5',
          f'electrodes-mae-{DATASET_NAME}-XS/10/5',
          f'electrodes-mae-{DATASET_NAME}-S/10/5',
      ],
  )
  all_variant_config = [experiment_name]
  for key in get_all_config_key():

    def get_config_value_by_experiment_name(
        experiment_name: str, key: str = key
    ):
      variant = experiment_name.split('-')[-1]
      config = map_variant_to_config(variant)
      return config[key]

    t = hyper.placeholder(
        experiment_name,
        key,
        post_process=get_config_value_by_experiment_name,
    )
    all_variant_config.append(t)

  all_variant_config = hyper.zipit(all_variant_config)
