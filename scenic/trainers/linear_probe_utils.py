...
def train_step(
  ...
  def loss_fn(params):
    ...
    if FOCAL_LOSS:
      focal_loss = scenic_model_utils.focal_softmax_cross_entropy(
          logits=logits,
          one_hot_targets=batch['label'],
          weights=batch['batch_mask'],
...
class LinearEvaluator:
  ...
  def __init__(
      ...
      representation_fn: Callable[
          [train_utils.TrainState, Batch], Tuple[jnp.ndarray, Any, Any]
      ],
      rng: jnp.ndarray,
  ...
  def _get_dataset(
      self, ds_name: str, config: ml_collections.ConfigDict, rng: jnp.ndarray
  ) -> dataset_utils.Dataset:
    """Lazy-loads given dataset."""
    # if 'lsm_300min_mood_vs_activity' in ds_name:
    #   dataset = lsm_mood_vs_activity_dataset.get_dataset(config, rng)
    # elif 'lsm' in ds_name:
    #   dataset = lsm_tiny_dataset.get_dataset(config, rng)
    # else:
    #   raise ValueError(f'Unknown dataset: {ds_name}')

    dataset = None
    raise ValueError(
        'Linear probe utils linear probing is not currently supported.'
    )
  ...
  def _train(
    ...
    dummy_reprs = representation_fn(
        {'inputs': jnp.zeros(input_shape), 'label': None, 'batch_mask': None}
    )
    ...
    total_steps, _ = train_utils.get_num_training_steps(
    ...
    for step in range(total_steps):
      ...
      if (
          writer
          and self.config.get('log_train_summary_steps', 0) > 0
          and step % self.config.log_train_summary_steps == 0
      ):
        train_utils.log_train_summary(
            ...
            extra_training_logs=jax.tree_util.tree_map(
                jax.device_get, extra_train_logs
            ),
            writer=writer,
            prefix=prefix,
            key_separator='/',
  ...
  def _eval(
    ...
    total_eval_steps = int(
        np.ceil(dataset.meta_data['num_val_examples'] / eval_batch_size)
    )
  ...
  def run_one_dataset(
    ...
    if not num_classes or not target_is_onehot:
      raise ValueError(
          'Linear probe datasets are required to have a positive integer number'
          ' of classes, and must also be one-hot encoded. num_classes: '
          f'{num_classes}, target_is_onehot: {target_is_onehot}'
      )
    ...
    logging.info(
        '[linear_eval]: Evaluating linear probe for dataset %s', ds_name
    )
  ...
  def run_all(
    ...
    def representation_fn(batch: Batch) -> jnp.ndarray:
      return self.representation_fn(
          jax_utils.unreplicate(repr_train_state), batch
      )[
          0
    ...
    for ds_name, ds_cfg in datasets.items():
      ...
      results[ds_name] = self.run_one_dataset(
          ds_name, ds_cfg, representation_fn, ds_rng, writer, repr_step
      )
