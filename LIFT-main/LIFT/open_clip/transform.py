...
from torchvision.transforms import Normalize, Compose, RandomResizedCrop, InterpolationMode, ToTensor, Resize, \
    CenterCrop, ColorJitter, Grayscale, Lambda
...
def image_transform_gpu_wrapper(image_size, mean, std, resize_mode, interpolation, aug_cfg):
    """Wrapper for image_transform_gpu, which returns a callable waiting for the device argument.
    This callable will be instantiated at the start of training and return a simpler version of
    image preprocessing transform that discard some transforms that CLIP and LIFT do not use.
    The preprocess is now optimized for GPU execution. It is now done after data is loaded.
    """
    def image_transform_gpu(device, input_dtype):
        nonlocal image_size, mean, std, resize_mode, interpolation, aug_cfg
        ...
        train_transform.extend([
            ToTensor(),
            Lambda(lambda x: x.to(device=device)),
        ])
        train_transform.extend([
            RandomResizedCrop(
                image_size,
        ...
        train_transform.append(Lambda(lambda x: x.to(dtype=input_dtype))) # change data type after resize to avoid any precision mismatch
        train_transform = Compose(train_transform)

        return train_transform
    # return a callable to be used during training
    return image_transform_gpu
...
def image_transform_v3(
    ...
    The preprocess is now optimized for GPU execution. It is now done after data is loaded.
    """
    return image_transform_gpu_wrapper(
        image_size=cfg.size,
        mean=cfg.mean,
        std=cfg.std,
        interpolation=cfg.interpolation,
