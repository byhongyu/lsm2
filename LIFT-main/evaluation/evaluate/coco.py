...
def zeroshot_parse_args(args):
    parser = argparse.ArgumentParser('COCO Zero-shot Evaluation')
    ...
    parser.add_argument("--data_path", default='/public/datasets/COCO2017', type=str, help="The path to the evaluation dataset")
...
def main(args):
    ...
    coco_root = os.path.join(args.data_path, "val2017")
    coco_annotations = os.path.join(args.data_path, "annotations", "captions_val2017.json")
    coco = CocoCaptions(root=coco_root, annFile=coco_annotations, transform=None)
    images = [transform_val(image) for image, _ in coco]
    captions = [caption[0] for _, caption in coco]

    # Load LIFT or CLIP model
    model = load_models(args, device)

    # Generate caption and image embeddings
    ...
    print(f"Top-{args.topk} I2T COCO Accuracy: {topk_accuracy * 100:.2f}%")
    topk_accuracy = calculate_topk_accuracy(captions_embeddings, images_embeddings, k=args.topk)
    print(f"Top-{args.topk} T2I COCO Accuracy: {topk_accuracy * 100:.2f}%")
