...
def remap_label_to_walk_example(example, dataset_name):
  """Remap hiking and treadmill to walking."""
  exercise_log = tf.cast(example['metadata']['log_value'], tf.int16)
  log_value_offset = tf.cast(
      dataset_constants.lsm_dataset_constants[dataset_name.split('/')[-1]][
          'label_value_offset'
      ],
      tf.int16
  )

  offset_log_val = exercise_log + log_value_offset
  map_to_walk = [90012, 90019]
  if tf.reduce_any(tf.equal(offset_log_val, map_to_walk)):
    example['metadata']['log_value'] = 90013 - log_value_offset

  return example


def fewshot_filter_examples(ds, n, allowed_labels):
  """Take n samples for each allowed class."""

  def filter_by_class(class_label):
    return lambda x: tf.equal(
        x['metadata']['log_value'], tf.cast(class_label, tf.int16)
    )
...
def update_metadata(
  ...
  # 6. Add label counts.
  label_counts = dataset_constants.lsm_dataset_constants[dataset_name].get(
      'label_counts', None
  )
  label_counts = [
  ...
  if filter_examples:
    metadata_update['num_train_examples'] = int(
        tf.math.reduce_sum(label_counts)
    )
...
def get_electrodes_activity_dataset(
    *,
    config,
    num_shards,
    batch_size,
  ...
  """Gets and formats the Electrodes Activity dataset.
  ...
  filter_examples = dataset_constants.lsm_dataset_constants[dataset_key].get(
  ...
  if filter_examples:
    ...
    raise ValueError('filter_examples must be True.')
  ...
  # 1a. Remap examples
  remap_fn = functools.partial(
      remap_label_to_walk_example, dataset_name=dataset_name
  )

  # 1b. Filter labels (Optional)
  filter_fn = functools.partial(
      lsm_dataset_utils.filter_log_values, allowed_labels=allowed_labels_offset
  )

  # 1c. Fewshot filter examples.
  samples_per_class = config.fewshot_samples_per_class
  fewshot_filter_fn = functools.partial(
      fewshot_filter_examples,
      n=samples_per_class,
      allowed_labels=allowed_labels_offset
  )
  ...
  train_ds = train_ds.map(
      remap_fn, num_parallel_calls=tf.data.experimental.AUTOTUNE
  )
  val_ds = val_ds.map(
      remap_fn, num_parallel_calls=tf.data.experimental.AUTOTUNE
  )

  # 1b. Filter examples by log value.
  if filter_examples:
    train_ds = train_ds.filter(filter_fn)
    val_ds = val_ds.filter(filter_fn)

  # 1c. Fewshot filter examples in train set
  ...
  meta_data = {
      ...
      'num_train_examples': int(len(allowed_labels_offset) * samples_per_class),
      'num_val_examples': dataset_utils.get_num_examples(
          dataset=loaded_dataset_name, split=val_split, data_dir=data_dir
      ),
      'num_test_examples': 0,
