...
def preprocess_example(example, dataset_name, dtype=tf.float32):
  ...
  # Get label index
  if (exercise_log) and (not mood_log):
    label_idx = 1
  elif (not exercise_log) and (mood_log):
    label_idx = 0
  else:
    label_idx = -1

  # Two classes: 0 for mood, 1 for exercise. This are later OHE'd
  return {
      'input_signal': features,
      'datetime_signal': time_features,
      'label': tf.one_hot(label_idx, 2),
...
def get_electrodes_mood_vs_activity_dataset(
  ...
  meta_data = {
      ...
      'num_train_examples': train_num_act_samples + train_num_mood_samples,
