...
def age_bin_example(example):
  """Bin age into 4 buckets."""
  ...
  example['metadata']['binned_age'] = age_bin
...
def preprocess_example(example, dataset_name, dtype=tf.float32):
  ...
  # Gender Labels:
  log_value = tf.cast(example['metadata']['binned_age'], tf.int32)
...
def get_electrodes_age_dataset(
    *,
    config,
    num_shards,
    batch_size,
  ...
  # 1b. Bin age into 4 buckets.
  train_ds = train_ds.map(
      age_bin_example, num_parallel_calls=tf.data.experimental.AUTOTUNE
  )
  val_ds = val_ds.map(
      age_bin_example, num_parallel_calls=tf.data.experimental.AUTOTUNE
  )
  ...
  meta_data = {
      ...
      'num_train_examples': dataset_constants.lsm_dataset_constants[
          loaded_dataset_key]['num_train_examples'],
      'num_val_examples': dataset_constants.lsm_dataset_constants[
          loaded_dataset_key]['num_test_examples'],
      'num_test_examples': 0,
