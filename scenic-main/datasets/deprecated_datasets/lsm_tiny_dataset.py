...
def update_metadata(
  ...
  # Get label weights from data constants.
  else:
    metadata_update['label_weights'] = dataset_constants.lsm_dataset_constants[
        dataset_name
    ].get('label_weights', None)

  # 6. Add label counts.
  metadata_update['label_counts'] = dataset_constants.lsm_dataset_constants[
      dataset_name
  ].get('label_counts', None)
...
def get_electrodes_dataset(
  ...
  # 1. Filter examples by log value.
  if filter_examples:
