...
--config=experimental/largesensormodels/scenic/configs/lsm_v2_linearprobe_finetune/lsm_v2_metabolic.py \
--platform=glp_8x16 \
--exp_name=lsm_v2_mae_small_metabolic_linearprobe \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/lsm_v2_linearprobe_finetune/lsm_v2_metabolic.py:runlocal
"""
...
XID = [159136233]  # LSM V2 Job (from Xin)
# XID = [159279932]  # LSM V2 w/ Naive Imputation + Mixed Masking Job (from Xin)
# WID = [1, 2, 3, 4, 5]
WID = [5]
...
PATCH_CONFIG = LSM_PREDEFINED_CONFIGS[
    f'{VARIANT}__patcher_config__10by1_sharedpatch'
]

# Attention masking (LSMv2)
MASK_CONFIG = LSM_PREDEFINED_CONFIGS['downstream_inherited_attn_masking']
REPRESENTATION_POOLING_FN = ['attn_masked_mean']
...
METADATA_ENCODING_FN = ['none']  # 'identity', or 'none'
...
def get_config(runlocal=''):
  ...
  config.experiment_name = f'lsmv2_mae_linearprobe-{DATASET_NAME}'
