...
--config=experimental/largesensormodels/scenic/configs/fewshot_experiments/mae_lsm_base_fewshot_linear_probe.py \
--platform=vlp_1x1 \
--exp_name=lsm_mae_base_10x5res_fewshot_linear_probe \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/fewshot_experiments/mae_lsm_base_fewshot_linear_probe.py:runlocal
"""
...
XID = 124248847
WID = 5
INIT_CHECKPOINT_DIR = f'/cns/dz-d/home/xliucs/lsm/xm/{XID}/{WID}'
# Specify a checkpoint step. (None defaults to the last check)
INIT_CHECKPOINT_STEP = None

# 1) Dataset variables and FEWSHOT SAMPLES PER CLASS.

# DATASET_NAME = 'fewshot_lsm_300min_600_activities_remapped_8class'
# SAMPLES_PER_CLASS = 20  # 1, 5, 10, 15, 20
# NUM_TRAIN_STEPS = 960  # 48, 240, 480, 720, 960
...
# 4) Embeddeding Model variant / patch H (time steps) / patch W (features)
VARIANT = 'B/10/5'
...
# 5) Other classification parameters.
WEIGHTED_LOSS = [False]  # whether or not to use label weights
LABEL_SMOOTHING = None

# 6) Logging variables.
LOG_EVAL_SUMMARY_STEPS = 16  # STEPS_PER_EPOCH
LOG_CHECKPOINT_STEPS = 16  # LOG_EVAL_SUMMARY_STEPS * 5
...
def get_config(runlocal=''):
  ...
  config.init_from.checkpoint_dir = INIT_CHECKPOINT_DIR
  config.init_from.checkpoint_step = INIT_CHECKPOINT_STEP

  config.fewshot_samples_per_class = SAMPLES_PER_CLASS
...
def get_hyper(hyper):
  ...
  return hyper.product([
      ...
      # hyper.sweep(
      #     'config.representation_pooling.time_window',
      #     REPRESENTAION_POOLING_WINDOW
      # ),
      # hyper.sweep('config.dataset_configs.label_weights', LABEL_WEIGHTS),
  ])
