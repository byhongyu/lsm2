...
--config=experimental/largesensormodels/scenic/configs/mae_lsm_base_patch_size_sweep.py \
--platform=vlp_4x8 \
--exp_name=lsm_mae_tier2_base_10_5_res_patch_size_sweep \
...
--uptc="" \
--binary=//experimental/largesensormodels/scenic:main \
--config=$(pwd)/experimental/largesensormodels/scenic/configs/mae_lsm_base_patch_size_sweep.py:runlocal
"""
...
LOG_CHECKPOINT_STEPS = 1000  # LOG_EVAL_SUMMARY_STEPS
MAX_NUM_CHECKPOINTS = int(NUM_TRAIN_STEPS / LOG_CHECKPOINT_STEPS)

# Model variant / patch H (time steps) / patch W (features)
VARIANT = 'B/10/5'
TOKEN_MASK_PROBS = ['constant_0.8']
LOSS_ONLY_MASKED_TOKENS = True

# Patch size sweep.
# NOTE: The below are the patch sizes we want to sweep.
# However, different patch dimension result in a different number of total
# patches, and thus differing needed compute. For this reason we split them up
# Into 4 classes by the patch area (EXTRA_SMALL, SMALL, MEDIUM, LARGER)
# SWEPT_TIME_PATCHES = [(5, 5), (10, 5), (20, 5), (30, 5)]
# SWEPT_SENSOR_PATCHES = [(10, 1), (10, 2), (10, 10), (10, 26)]

EXTRA_SMALL_PATCH_SIZES = [(10, 1)]  # requires 16x16 VLP
SMALL_PATCH_SIZES = [(5, 5), (10, 2)]  # requires 8x8 VLP
MEDIUM_PATCH_SIZES = [(10, 10), (20, 5), (30, 5)]  # requires 4x8 VLP
LARGE_PATCH_SIZES = [(10, 26)]  # requires 4x4 VLP
SWEPT_PATCH_SIZES = MEDIUM_PATCH_SIZES
...
def get_config(runlocal=''):
  ...
  config.masked_feature_loss.token_mask_probability = TOKEN_MASK_PROBS[-1]
...
def get_hyper(hyper):
  ...
  return hyper.product([
      hyper.sweep('config.model.patches.size', SWEPT_PATCH_SIZES),
      hyper.sweep(
          'config.masked_feature_loss.token_mask_probability', TOKEN_MASK_PROBS
      ),
